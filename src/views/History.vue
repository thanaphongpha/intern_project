<template>
  <div class="container">
    <div class="row d-flex justify-center">
      <h2>ประวัติเวลาเข้าทำงาน</h2> {{user}}
    </div>
<!--    <div class="row">-->
          ...
          .then((respone) => {
            if (respone.status === 200) {
              ...
              for (let i = 0; i < this.historyData.length; i++) {
                ...
                const dateIn = this.historyData[i].start
                const first = dateIn
                const second = this.historyData[i].end
                events.push({
                  name: 'เวลาเข้า '+ moment(this.historyData[i].start).format('hh:mm:ss a'),
                  start: first,
                  end: second,
                  color: this.colors[4],
                  timed: !allDay,
                ...
                if (this.historyData[i].end !== null) {
                  events.push({
                    name: 'เวลาออก '+moment(this.historyData[i].end).format('hh:mm:ss a'),
                    start: first,
                    end: second,
                    color: this.colors[7],
                    timed: !allDay,
